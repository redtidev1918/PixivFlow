...
- **多平台扇出（fan-out）**：`targets[].delivery.targets` 声明多个交付目标时，同一作品按路由各写一条独立意图 + 独立 outbox 行（路由解析唯一入口 `src/delivery/targetRoutes.ts`）。`DeliveryService.enqueue` 返回的 `duplicate` 是**所有路由的 AND**——只有每个平台都确认才算“已投递”；候选期去重由 `deliveredIdsForAllTargets` / `submittedIdsForAllTargets` 用一条 `GROUP BY pixiv_id HAVING COUNT(DISTINCT delivery_target) >= N` 查询判断。`DeliveryLedgerPort` 把 cell 状态聚合到全部路由（全确认→`confirmed`；任一路由仍可执行→`live`；无确认且有 dead/未入队→`lost`），不新增 `partial` 状态。运维通知只走主路由（`primaryDeliveryName`），保持“一个 occurrence 一条终态通知”。终态 outbox 行在投递意图仍未收敛时由 `OutboxRepository.revive` 重开，保证重试只重发失败平台。详见 [投递运行时架构](architecture/delivery-runtime.md)。
- **Outbox 边界**：outbox 只保证“已产生的投递”最终送达，重放**永不**重新进入候选选择。
- **能力与内容模型（capability / Content）**：投递形态按**能力**而不是平台名决定。`delivery.targets.<name>.capabilities` 是可选声明（布尔能力 + 上限，数值只能收紧不能放宽），缺省用平台内置档案（`httpMultipart` 不设上限；`telegram` caption ≤1024、上传 ≤50MB、album 2–10）；未知 type 解析为保守的 text-only 档案，配置校验才负责拒绝拼错的 type。平台无关的 `Content`（text/image/file/video/album 组合，`src/delivery/content.ts`）在 enqueue 时冻结进 outbox payload，旧行缺字段时由 provider 用 `files` + `context` 重建，**不需要数据迁移**。`planDelivery` 是唯一下降点：album 受能力或区间限制时展开为单项并记 `downgrades`，目标完全不支持的媒体进 `unsupported`（adapter 必须上报，绝不静默丢弃）；超过 `album.max` 只展开，分批由 adapter 负责。详见 [投递运行时架构](architecture/delivery-runtime.md)。
- **Readiness 边界**：delivery target 配置 `readinessUrl` 后，非 2xx 只释放租约并延后消费，attempt 不增加；`/live` 不能代替 `/ready`。
- **网关边界（Messaging Gateway Client）**：`delivery.targets.<name>.type` 现有三种——`httpMultipart`（TelePost 式 multipart）、`telegram`（审核链，媒体不离开 Telegram）、`webhook`（**平台无关**的通用网关：POST 一份统一消息 JSON，`reference` 发本机绝对路径或 `base64` 内联字节，可选 HMAC 签名 `sha256=HMAC-SHA256(secret,"<ts>.<rawBody>")`）。PixivFlow **只做网关客户端**：不实现 QQ/微信/飞书/Discord 协议、不做扫码登录、不生成配对二维码、不保存平台登录信息；平台生态交给社区网关（OneBot 实现 / AstrBot / Hermes / 自建服务）。webhook 的 ACK 语义独立于 TelePost 信封（`parseWebhookAck`）：2xx 但状态词不认识按可重试处理，终态失败绝不记为成功，传输失败 throw 交给 outbox。`GET /api/gateways` 是只读投影（endpoint 经 `redactUrl`、`pairingSupported` 表示该路由是否配了 `pairingUrl`、`connectionStatus` 是网关侧真值的缓存投影）。配对由**网关**承担：`GET /api/gateways/:name/pairing` 只把网关自己的配对端点报文**原样透传**渲染（PixivFlow 不生成二维码、不说平台登录协议、不持有会话、不落库；默认 `redirect: manual`，非 2xx 一律 `pairable:false`）。详见 [投递运行时架构](architecture/delivery-runtime.md)。 运维入口：`pixivflow gateway list|status|test` 与 `pixivflow delivery status|retry`（`retry` 默认预览、需 `--yes`，只重开仍欠投递的路由；`gateway test` 只证明端点是否应答，绝不当作投递成功，写入 `gateway_connections` 的 endpoint 已脱敏）。路由解析唯一入口是 `src/delivery/gatewayRoutes.ts`（CLI 与 WebUI 投影共用，避免两处对「已配置网关」的定义漂移）。详见 [投递运行时架构](architecture/delivery-runtime.md)。
...
| `gateway_connections` | `name` UNIQUE、`type`、`endpoint`、`status`、`metadata` | 外部消息网关的**指针**（不是凭据库）：`status` 是网关侧配对真值的缓存投影，允许 stale；PixivFlow 不保存任何平台登录信息 |
...
- [pixivflow-webui 前端仓库](https://github.com/redtidev1918/pixivflow-webui) —— WebUI 的模块地图与状态管理详见其 [DEVELOPMENT_GUIDE](https://github.com/redtidev1918/pixivflow-webui/blob/master/docs/DEVELOPMENT_GUIDE.md)
