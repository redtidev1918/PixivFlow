...
- **网关边界（Messaging Gateway Client）**：`delivery.targets.<name>.type` 现有三种——`httpMultipart`（TelePost 式 multipart）、`telegram`（审核链，媒体不离开 Telegram）、`webhook`（**平台无关**的通用网关：POST 一份统一消息 JSON，`reference` 发本机绝对路径或 `base64` 内联字节，可选 HMAC 签名 `sha256=HMAC-SHA256(secret,"<ts>.<rawBody>")`）。PixivFlow **只做网关客户端**：不实现 QQ/微信/飞书/Discord 协议、不做扫码登录、不生成配对二维码、不保存平台登录信息；平台生态交给社区网关（OneBot 实现 / AstrBot / Hermes / 自建服务）。webhook 的 ACK 语义独立于 TelePost 信封（`parseWebhookAck`）：2xx 但状态词不认识按可重试处理，终态失败绝不记为成功，传输失败 throw 交给 outbox。`GET /api/gateways` 是只读投影（endpoint 经 `redactUrl`、`pairingSupported:false`、`connectionStatus` 是网关侧真值的缓存投影）。详见 [投递运行时架构](architecture/delivery-runtime.md)。
