...
- **打开下载目录（宿主可选增强，后端为基线）**：新增只读端点 `POST /api/files/reveal`（body `{path?, type?, resolveOnly?}`）。后端在本机拉起系统文件管理器——只有后端与用户同一台机器时才对；后端在服务器 / 容器里必须如实返回 `FILE_REVEAL_UNSUPPORTED`（HTTP 200，不是错误），由前端降级为「复制路径」。宿主若要让文件夹在前台打开，可在自己的 bridge 上提供 `openDirectory(path)`：WebUI 先用 `reveal` 的 `resolveOnly` 拿到后端收敛过的目录，再调宿主；宿主只需 `open`/`explorer`/`xdg-open` 一个已存在的目录，**不解析、不校验、不接触任何业务配置**。`reveal` 不校验 Pixiv 凭据（占位 token 的机器也能打开下载目录），路径越界一律 400 `FILE_PATH_INVALID`，不存在 404 `FILE_NOT_FOUND`。以上为追加式新增，既有端点行为不变。
