...
> live in `package.json`; the root `CHANGELOG.md` is generated by
> release-please, and a CI workflow cross-checks package.json, npm and
> git tags. The standard path is: push a `vX.Y.Z` tag and let the Publish
> workflow release via npm Trusted Publishing (OIDC). A local fallback via
> `scripts/publish.sh` and post-release verification steps are included.
...
- [ ] [CHANGELOG](https://github.com/redtidev1918/PixivFlow/blob/master/CHANGELOG.md) 已有该版本条目——由 release-please 自动生成,不要手写;`create-release.sh` 按 `## [<version>]` 模式提取 Release 说明。
...
`create-release.sh`:检查 gh CLI 与登录状态 → 要求远程已存在对应标签(可现场推送)→ 从根 `CHANGELOG.md` 提取 `## [<version>]` 块作为说明 → 交互确认后创建或更新 Release。批量补历史标签用 `create-releases-for-tags.sh`。

脚本内 REPO 变量已修正为 `redtidev1918/PixivFlow`,直接运行即可在正确仓库创建 Release。也可跳过脚本:

```bash
gh release create vX.Y.Z --repo redtidev1918/PixivFlow --generate-notes
```
...
- GitHub Pages 文档由 `static.yml` 单独驱动,push 到 master 且改动 docs/** 时部署,与 npm 发布相互独立。
