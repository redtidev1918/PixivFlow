...
|---|---|---|---|---|---|---|
| **PixivFlow** `redtidev1918/PixivFlow` | Node/TS + Express + Socket.IO + node:sqlite，**2.46.0**（HEAD `84078e3`） | 执行平面：抓取/调度/下载/交付 outbox；**不触碰 Telegram 凭据**（SI-1） | WebUI :3000 — 健康 `*/api/health,/health`；运行时契约 `*/api/status,/status,/api/version,/version`（P1 已落地，免认证）；`/api/auth(7)/config(16)/download(17)/stats(4)/logs(2)/files(4)/scheduler(5)/admin/*`；Socket.IO 日志流+下载状态。时钟面 :8090 —— `GET /health`、`/internal/schedules`、`POST /internal/schedules/:id/run`、`POST /internal/targets/:id/refetch`、`GET/POST recover`、`POST /internal/outbox/drain`（bearer `SCHEDULER_TRIGGER_TOKEN` / refetch 用 `PIXIVFLOW_REFETCH_TOKEN`，缺失 fail-closed；202/202/200/503） | SQLite `./data/pixiv-downloader.db`（16 表：`schedule_slots` 槽账本、`schedule_slot_items` 幂等 cell、`deliveries`、`outbox` 持久事务、`tokens`、`config_history`…，WAL） | **无 `schemaVersion`**；缺失即自举 `generateDefaultConfig` | **无 DB 版本/迁移框架**——`DatabaseMigration` 纯声明式幂等（建表+列探测 ALTER） |
...
- **宿主内登录（F4.1 新增）**：交互式登录原先只能由后端拉起系统浏览器（`puppeteer-core` + `pixiv-token-getter` 解析 Chrome），而宿主自身的 webview（macOS 为 WKWebView）无法被 puppeteer/CDP 驱动，因此新增两步宿主内流程：`POST /api/auth/login/host/start` → `{loginId, authUrl, redirectUri}`；宿主用自己的窗口加载 `authUrl` 并监听跳转到 `redirectUri`，把 URL 中的 `code` 交回；`POST /api/auth/login/host/complete`（`{loginId, code}` 或 `{loginId, callbackUrl}`）换取令牌并写回配置，响应与 `POST /api/auth/login` 同形。约定：PKCE `code_verifier` 只在后端、会话 10 分钟 TTL 且一次性消费；`code` 缺失时返回 `AUTH_CODE_REQUIRED` 且**不**消费会话，重复/过期 `loginId` 返回 `AUTH_HOST_LOGIN_SESSION_INVALID`；宿主只需提供「打开窗口 + 回传 code」，不构造任何 Pixiv URL、不接触令牌。以上均为追加式新增，`POST /api/auth/login`（puppeteer 路径）行为不变。
- **网络路径由宿主负责（token 交换在后端，F4.1 实测）**：`host/complete` 的 token 交换由**后端进程**发起，宿主只回传 `code`。后端只认 `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY`（`src/config/environment.ts` 读取并映射到 `network.proxy`，因此下载也走同一条路径）。实测：本机直连 `oauth.secure.pixiv.net` 12 秒超时，经代理 0.5 秒返回 400。从 Finder / Dock 启动的宿主进程不继承任何环境变量，因此宿主必须自己把系统代理转成上述环境变量注入后端 env（且不覆盖用户已设置的值）；宿主 webview 走系统代理而后端无代理 = 登录交换必然失败。此条与「环境契约只有 `PORT`/`HOST`/`STATIC_PATH`」不冲突：代理变量不是业务配置，而是宿主对网络可达性的保证。
- **系统集成能力属于宿主层，不属于 PixivFlow 运行时**（架构规则）：Finder/Explorer 打开、系统通知、剪贴板、托盘、开机自启、文件关联——凡是"把信息交给用户设备上某个界面"的动作，都由宿主实现，运行时**绝不** spawn 一个桌面程序。理由不是"后端不能 `open`"，而是语义：`浏览器 → Fly.io 后端 → xdg-open` 这条链在服务器上没有任何意义，却会让用户以为"WebUI 里的打开文件夹 = 打开我电脑上的文件夹"。运行时只回答**位置**。
- **位置查询（后端职责边界）**：`GET /api/files/location?type=&path=` 返回 `{path, directory, exists, isDirectory}` —— 路径规范化、越界拦截（含 `/downloads-out` 这类同前缀兄弟目录 → 400 `FILE_PATH_INVALID`）、存在性如实上报（文件已删仍 200 + `exists: false`，历史记录因此仍可复制路径）。**无副作用**：不 `open`、不 `explorer`、不 `xdg-open`；也不校验 Pixiv 凭据（占位 token 的机器照样能查到下载目录）。
- **宿主实现"在文件管理器中显示"**：宿主在自己的 bridge 上暴露 `revealPath(path)` —— 语义是**定位文件**而不是"打开目录"，与 Finder 的 Show in Finder 一致：macOS `open -R <file>`、Windows `explorer /select,<file>`、Linux `xdg-open <目录>`。宿主只做"校验路径存在 + 交给平台打开器"，**不解析、不校验、不接触任何业务配置**（路径收敛在运行时那一步已完成）。**没有宿主的纯浏览器形态**（Docker / NAS / VPS / Fly.io）不隐藏该动作，而是降级为「复制路径」——对这些部署，复制路径才是真正可用的动作（SSH / SMB / rsync）。
- 以上为追加式新增，既有端点行为不变。
